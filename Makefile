...
help:  ## 顯示可用指令
	...
	@echo "  make reseed       # 全量重建（down -v → volumes 全清 → 重新 up）"
	@echo ""
...
reseed:  ## 全量重建（清空所有 volumes：mssql/kafka/rabbitmq/influxdb）
	@# 為什麼需要 reseed：
	@# - prefix 規格或 schema 調整時，Kafka/RabbitMQ 的舊資料與 DB 資料可能互相影響，導致 demo 混雜舊格式；
	@#   用 down -v 重新拉起可確保端到端完全乾淨、可重現。
	@if [ "$(CONFIRM)" != "YES" ]; then \
	  echo "!! DANGER: 這會清空所有 docker volumes（mssql/kafka/rabbitmq/influxdb），資料會全消失"; \
	  echo "!! 若你確定要全量重置，請改用：make reseed CONFIRM=YES"; \
	  exit 1; \
	fi
	$(COMPOSE) down -v
	$(COMPOSE) up -d
	@echo ">> 已完成 reseed（全量重建）"
...
smoke:  ## 打幾個關鍵 API，確認 backend / domain profile / 4 大模組活著
	...
	@echo ">> ncrs (最近 5 筆)"
	@curl -fsS "http://localhost:8080/api/ncrs?take=5" | head -c 300 && echo
