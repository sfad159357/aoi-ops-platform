...
即時計算 SPC 八大規則 / Cpk，並以事件驅動方式串接 **不良單（NCR）/ 異常記錄 / 物料追溯查詢**。

主軸：**即時生產 → 儲存 → 消費 → API 回傳 → 即時監控 → 即時運算 → 業務模組 → 可追溯查表**。

> 同一份 codebase 透過 Domain Profile 機制，可在 **ABF / 半導體** 兩種產業 demo 之間切換用語與規格，
...
| ② **不良單（NCR）** | RabbitMQ `ncr` | SignalR `/hubs/ncr` + REST 預載 | `NcrsPage` |
| ③ **異常記錄** | RabbitMQ `alert`（由 Kafka `aoi.defect.event` 路由而來） | SignalR `/hubs/alarm` + REST 預載 | `AlarmsPage` |
| ④ **物料追溯查詢** | SQL Server（panel_no / 物料批號 / 同 lot） | REST `GET /api/trace/panel/{panelNo}` / `GET /api/trace/material-tracking?date=YYYY-MM-DD&take=20` | `TraceabilityPage` |
...
| 即時推送 | **.NET Core SignalR**（WebSocket，自動降級 SSE / Long Polling） | 3 個 Hub：spc / alarm / ncr（trace 走 REST 查詢） |
| 事件串流 | Apache Kafka 3.x（KRaft mode） | 設備感測層 fan-out |
| 業務路由 | RabbitMQ 3 + AMQP | alert / ncr 業務佇列 |
...
⚡ Kafka（aoi.inspection.raw / aoi.defect.event）
   ...
   ├─▶ Python kafka-rabbitmq-publisher   ──▶ RabbitMQ（alert / ncr）
...
🐇 RabbitMQ
   ...
   └─ ncr ─▶ .NET NcrRabbitWorker ──▶ SQL Server ncrs + /hubs/ncr

🗄️ SQL Server ─▶ ASP.NET Core API（REST：/api/lots / alarms / ncrs / trace ...）
...
本專案沒有「啟動鍵」或快捷鍵；**務必先讓 Docker 引擎跑起來**，`make up` 才能連到 daemon。

1. 開啟 **Docker Desktop**，等到 whale icon 顯示為 Running（狀態列或選單「Docker Desktop is running」）。
2. 終端機確認：`docker info` 或 `docker ps` 應成功，不應出現 `Cannot connect to the Docker daemon`。

若看到 `Cannot connect to the Docker daemon at unix:///Users/.../docker.sock`，代表 Docker Desktop 未啟動或未登入完成；請先修好這一步再執行下方指令。
...
make reseed           # 清空所有 volumes（mssql/kafka/rabbitmq/influxdb），重建整套 demo（最乾淨）
```
...
### 常見錯誤排除（Docker Compose）

- **no configuration file provided: not found**
  - **原因**：compose 檔不在 repo 根目錄；本專案使用 `infra/docker/docker-compose.yml`
  - **解法**：用 `-f infra/docker/docker-compose.yml`（或直接 `make up`）

- **Bind for 0.0.0.0:9092 failed: port is already allocated**
  - **原因**：同一份 compose 被用不同 project name 啟動成兩套（兩邊都想綁 Kafka 9092）
  - **解法**：固定 project name（建議 `-p aoiops`），或把多餘 project 關掉
    - `docker compose -p aoiops -f infra/docker/docker-compose.yml up -d`
    - `docker compose -p <other> -f infra/docker/docker-compose.yml down -v`
...
curl -s http://localhost:8080/api/ncrs?take=5 | head -c 200; echo
curl -s "http://localhost:8080/api/trace/panels/recent?take=3"; echo
```

### 物料追蹤真資料查詢（給 DBeaver / 前端對帳）

「物料追蹤查詢」頁已改為直接查 SQL Server 真實交易資料（`panel_material_usage`），
預設 `take=20` 筆，可在前端下拉調整。

```bash
# 某一天的物料使用明細（前端同一支 API）
curl -s "http://localhost:8080/api/trace/material-tracking?date=2026-04-29&take=20"
```

若前端顯示 `material tracking failed: 404`，通常是 backend 容器仍在舊版映像，請重建 backend：

```bash
make backend-rebuild
```

> 為什麼要用 `make backend-rebuild`：固定 compose project 為 `aoiops`，可避免誤啟動第二套容器造成 1433 port 衝突或 API 版本不一致。
...
- **Demo 腳本（6 分鐘走完整條 ABF 產線）**：[docs/demo-script.md](docs/demo-script.md)
...
- 部署與維運 Runbook（面試版）：[docs/0503_03_deployment-runbook.md](docs/0503_03_deployment-runbook.md)
- ISA-95 + 業務閉環固定話術：[docs/0503_04_isa95-business-closure-talk-track.md](docs/0503_04_isa95-business-closure-talk-track.md)
- ISO 27001 / ISO 9001 控制項對照（面試版）：[docs/0503_05_iso27001_iso9001_control-mapping.md](docs/0503_05_iso27001_iso9001_control-mapping.md)
- 面試 60 秒口語稿（MES 版）：[docs/0503_06_interview-60s-talk-track.md](docs/0503_06_interview-60s-talk-track.md)
- MES 模擬面試問答（20 題）：[docs/0503_07_mes-interview-qa-20.md](docs/0503_07_mes-interview-qa-20.md)
- Domain Entities ↔ 資料表中文對照（含 Mermaid）：[docs/0503_08_domain-entity-table-zh-mapping.md](docs/0503_08_domain-entity-table-zh-mapping.md)
